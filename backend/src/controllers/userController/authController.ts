...
export const loginUsingDb = async (email: string, password: string) => {
  ...
  return {
    token,
    refreshToken,
    userData: { id: user.id, email: user.email, name: user.username },
  };
};
...
export const getUserProfile = async (req: Request, res: Response) => {
  res.status(200).json((req as any).user || null);
};
...
export const updateUserProfile = async (req: any, res: Response) => {
  try {
    ...
    if (profileImage)
      updateData.profileImage = `${process.env.CDN_BASE_URL || ""}/uploads/${
        profileImage.originalname
      }`;
    const updatedUser = await prisma.user.update({
      where: { id: req.user.id || req.user.sub || req.user._id },
      data: updateData,
      select: {
        id: true,
        username: true,
