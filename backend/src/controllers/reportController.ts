...
export const getReportsSummary = async (req: Request, res: Response) => {
  try {
    ...
    const [reports, total, statsResult] = await Promise.all([
      ...
      prisma.$transaction([
        ...
        prisma.report.groupBy({ by: ["itemType"], _count: true, orderBy: { _count: { itemType: "desc" } } }),
        prisma.report.groupBy({ by: ["reason"], _count: true, orderBy: { _count: { reason: "desc" } }, take: 5 }),
      ]),
    ]);
    ...
    const reportsWithItems = await Promise.all(
      reports.map(async (report: any) => {
        const models: Record<string, any> = {
          ...
          REAL_ESTATE: prisma.realEstate,
          CAR: prisma.car,
          BOAT: prisma.boat,
          MOTORCYCLE: prisma.motorcycle,
          TRAKTOR: prisma.farmequipment,
        };
        const model = models[report.itemType];
        const item = model
          ? await model.findUnique({ where: { id: report.itemId }, select: { id: true, title: true, price: true, images: true } }).catch(() => null)
          : null;
        return { ...report, item };
      }),
    );
    ...
    res.status(200).json({
      ...
      data: {
        ...
        stats: {
          total: statsResult[0],
          new: statsResult[1],
          inProgress: statsResult[2],
          done: statsResult[3],
          resolved: statsResult[4],
          closed: statsResult[5],
          byItemType: statsResult[6],
          topReasons: statsResult[7],
        },
      },
    });
  } catch (err: any) {
