  ...
  try {
    const res = await fetch("https://open.tiktokapis.com/v2/post/publish/content/init/", {
      ...
      body: JSON.stringify({
        ...
        source_info: { source: "PULL_FROM_URL", photo_images: [payload.imageUrl], photo_cover_index: 0 },
      }),
    });
    const data: { error?: { message?: string }; data?: { publish_id?: string } } = await res.json();
    if (!res.ok)
    ...
    return { platform: "tiktok", success: false, error: message };
  }
}
...
export async function postToFacebook(payload: SocialPostPayload): Promise<SocialPostResult> {
  ...
  try {
    ...
    const body = payload.imageUrl
      ? { url: payload.imageUrl, caption: message }
      : { message, link: payload.listingUrl };
    ...
    const data: { error?: { message?: string }; id?: string; post_id?: string } = await res.json();
    if (!res.ok)
    ...
    const message = err instanceof Error ? err.message : "Network error";
    return { platform: "facebook", success: false, error: message };
  }
}

export async function postToAllPlatforms(
  payload: SocialPostPayload,
  platforms: Array<"tiktok" | "facebook"> = ["tiktok", "facebook"],
