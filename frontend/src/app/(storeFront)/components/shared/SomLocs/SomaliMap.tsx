  ...
  return (
    <APIProvider apiKey={apiKey}>
      <div className="bg-white border rounded-xl shadow-sm p-4 w-full">
        <div className="flex justify-between items-end mb-4">
          ...
          {selectedRegion && (
            <button
              ...
              className="text-[10px] bg-gray-100 text-gray-500 px-2 py-1 rounded-md font-bold hover:text-red-600"
            >
              CLEAR FILTER
            </button>
          )}
        ...
        <div className="w-full h-[350px] rounded-xl border border-gray-100 overflow-hidden bg-gray-50">
          <Map
        ...
        <div className="mt-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {STATIC_REGIONS.map((regionName) => {
              const count = regionTotals[regionName] || 0;
              const isActive =
                selectedRegion?.toLowerCase() === regionName.toLowerCase();
              return (
                <button
                  key={regionName}
                ...
                >
                  {regionName}
                  <span
                    className={`text-[9px] px-1.5 py-0.5 rounded-md ${isActive ? "bg-blue-500" : "bg-gray-100 text-gray-400"}`}
                  >
                    {count}
                  </span>
                </button>
