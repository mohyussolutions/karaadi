...
export default function BoatsForm({
  ...
  const getFee = useCallback(
    (cat: string) =>
      activeFeeConfig ? Number(activeFeeConfig[BOAT_FEE_MAPPING[cat]] || 0) : 0,
    [activeFeeConfig],
  );
  const getNestedSubcategories = () => {
    if (!formData.category) return [];
    const k = CATEGORY_TO_NESTED_KEY[formData.category];
    return k && nesCategories.boatsNestedMap
      ? nesCategories.boatsNestedMap[k] || []
      : [];
  };
  ...
  const catOptions = BOAT_CATEGORIES.map((c) => ({
    value: c.key,
    label: t(c.labelKey),
  }));
  const subCatOptions = getNestedSubcategories().map((s: any) => ({
    value: s.labelKey || s,
    label: t(s.labelKey || s),
  }));
    ...
    <div className="rounded-3xl border border-gray-100 shadow-sm p-6 md:p-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {(
            [
              ["type", "Type", "e.g. Speedboat", true],
              ["boatModel", "Model", "e.g. 2023", true],
              ["color", "Color", "e.g. Blue", true],
