...
export default function MarketplaceForm({
  ...
  const [customCats, setCustomCats] = useState<
    {
      ...
      nameEn: string;
      nameSo: string;
      subcategories: { key: string; nameEn: string; nameSo: string }[];
    }[]
  >([]);
    ...
    const load = async () => {
      try {
        const [regs, cits, feeRes, catsRes] = await Promise.all([
          ...
          getAllCities(),
          getMarketplaceFees(),
          fetch(`${BASE_API_URL}/api/marketplace-categories`)
            .then((r) => (r.ok ? r.json() : []))
            .catch(() => []),
        ]);
        setRegions(regs || []);
  ...
  const descError = descTouched
    ? formData.description.length < 10
      ? t(
          "createMarketplace.descTooShort",
          "Description must be at least 10 characters",
        )
      : descHasLink
        ? t(
            "createMarketplace.descNoLinks",
  ...
  const getFee = useCallback(
    (cat: string) =>
      activeFeeConfig
        ? Number(activeFeeConfig[MARKETPLACE_FEE_MAPPING[cat]] || 0)
        : 0,
    [activeFeeConfig],
  );
  ...
  const getNestedSubcategories = (): { value: string; label: string }[] => {
    ...
    if (dbCat?.subcategories.length) {
      return dbCat.subcategories.map((s) => ({
        value: s.key,
        label: isSo ? s.nameSo : s.nameEn,
      }));
    }
    const hardcoded = (nesCategories.marketplaceNestedMap as any)[
      formData.category
    ];
    return (hardcoded || []).map((s: any) => ({
      value: s.key,
      label: t(s.labelKey, { defaultValue: s.name || s.key }),
    }));
  };
    ...
    try {
      ...
      if (result.success && createdId) {
        toast.update(toastId, {
          render: t("createMarketplace.successMessage"),
          type: "success",
          isLoading: false,
          autoClose: 2000,
        });
        ...
        toast.update(toastId, {
          render:
            result.error ||
            result.message ||
            t("createMarketplace.errorMessage"),
          type: "error",
          isLoading: false,
          autoClose: 3000,
        });
      ...
      toast.update(toastId, {
        render: t("createMarketplace.errorMessage"),
        type: "error",
        isLoading: false,
        autoClose: 3000,
      });
  ...
  const conditionOptions = CONDITION_KEYS.map((c) => ({
    value: c.value,
    label: t(c.key, { defaultValue: c.value }),
  }));
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
            {t("createMarketplace.mainCategoryLabel")}
          </label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
              {t("createMarketplace.categoryLabel")}
            </label>
            <SelectField
              ...
              options={catOptions}
              placeholder={t("createMarketplace.selectCategory")}
            />
          </div>
          <div className="space-y-2">
            <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
              {t("createMarketplace.subCategoryLabel")}
            </label>
            <SelectField
              value={formData.subCategory}
              onChange={(v) => setFormData({ ...formData, subCategory: v })}
              options={subCatOptions}
              placeholder={t("createMarketplace.selectSubcategory")}
        ...
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
            {t("createMarketplace.titleLabel")}
          </label>
          <input
            placeholder={t("createMarketplace.titlePlaceholder")}
        ...
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
            {t("createMarketplace.conditionLabel")}
          </label>
          <SelectField
            value={formData.condition}
            onChange={(v) => setFormData({ ...formData, condition: v })}
            options={conditionOptions}
            placeholder={t("createMarketplace.selectCondition")}
          />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
              {t("createMarketplace.descriptionLabel")}
            </label>
            <textarea
            ...
            />
            {descError && (
              <p className="text-xs font-bold text-red-500 ml-1">{descError}</p>
            )}
          ...
          <div className="space-y-2">
            <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
              {t("createMarketplace.regionLabel")}
            </label>
            <SelectField
              ...
              placeholder={t("createMarketplace.selectRegion")}
            />
          </div>
        </div>
        ...
        {businessId && (
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <label className="text-xs font-black text-gray-400 uppercase tracking-wider ml-1">
                {t("createMarketplace.websiteLabel")}
              </label>
              <span className="text-[10px] font-black text-blue-400 uppercase bg-blue-50 px-2 py-0.5 rounded-full">
                {t("createMarketplace.optional")}
              </span>
            </div>
            <input
              type="url"
              placeholder={t("createMarketplace.websitePlaceholder")}
        ...
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase tracking-widest ml-1">
            {t("createMarketplace.priceLabel")}
          </label>
        ...
        <div className="p-6 border-2 border-dashed border-gray-100 rounded-3xl bg-gray-50/50">
          <ImageUpload
            ...
            label={t("createMarketplace.upload")}
          />
        </div>
