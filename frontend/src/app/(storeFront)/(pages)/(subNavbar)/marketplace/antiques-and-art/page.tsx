...
import { AntiquesAndArtNestedSub } from "@/app/(links)/storeFrontLinks/mainCategotyCategorySubCategory";
  ...
  const filteredForHook = useMemo(() => {
    ...
    if (selectedSubcategory) {
      ...
      list = list.filter((item) => {
        ...
        return subs.some((stored) => {
          const s = String(stored || "");
          return s === selectedSubcategory || s.endsWith(`.${selectedKey}`) || s === selectedKey || s.toLowerCase().includes(t(selectedSubcategory).toLowerCase());
        });
      });
    }
    ...
    return list;
  }, [allAntiquesItems, selectedSubcategory, selectedRegion, checkedCities, t]);

  const ordered = useListingFeed(filteredForHook, query, "Antiques & Art");
