...
export default function CarParts({ initialData = [] }: { initialData?: any[] }) {
  ...
  const subCategoryLinks = useMemo(() => {
    return nesCategories.carsNestedData?.CarPartsNestedSub || [];
  }, []);
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5 sm:gap-3">
                {itemsToDisplay.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-20 text-gray-400 text-xs font-medium">
                    {t("noPartsFound", {
                      defaultValue:
                        "Lama helin wax qeybood ah oo waafaqsan raadintaada.",
                    })}
                  </div>
                )}
              </div>
