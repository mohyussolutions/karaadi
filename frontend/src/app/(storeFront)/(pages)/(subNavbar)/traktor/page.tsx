...
function TractorLinks() {
    ...
    return Array.isArray(items)
      ? items.filter(
          (item: any) =>
            ...
            item.category.includes("Tractor"),
        )
      : [];
  }, [items]);
  ...
  const regionCityCounts = useMemo(() => {
    ...
    allTractorItems.forEach((item: any) => {
      if (item.region) {
        ...
        if (selectedCities.length > 0) {
          ...
          for (const city of selectedCities) {
            let filterParams: any = {
              city: city,
              category: "Tractor", // Adjust this based on your category
            };
            ...
            const uniqueItems = results.filter(
      ...
      return allTractorItems.filter((item: any) => {
        return (
          ...
          item.subcategory.some(
            (sub: string) => sub.toLowerCase() === selectedCategory,
          )
        );
      });
    }
  ...
  if (error)
    return (
      <div className="text-red-500 p-4">
        Cilad ayaa ku timid soo dejinta tractorada.
      </div>
    );
              ...
              {itemsToDisplay.length > 0 ? (
                ...
                <div className="col-span-full text-center py-10 text-gray-500">
                  {selectedRegion || Object.keys(checkedCities).length > 0
                    ? `Ma jiro tractor ${selectedRegion ? `gobolka ${selectedRegion}` : ""}${Object.keys(checkedCities).length > 0 ? ` magaalooyinka ${Object.keys(checkedCities).join(", ")}` : ""}`
                    : query.trim()
                      ? `Ma jiro wax tractor ah la raadinayay "${query}"`
                      : "Ma jiro wax tractor ah oo la helay"}
                </div>
              )}
            </div>
          )}
