...
function TractorForSale() {
  ...
  // 1. Shaandheyn Aasaasi ah: Soo qaado kaliya shayada ku jira "Tractor for Sale"
  const allTractorsForSaleItems = useMemo(() => {
    return Array.isArray(items)
      ? items.filter(
          (item: any) =>
            item.category &&
            Array.isArray(item.category) &&
            item.category.includes("Tractor for Sale") // Isticmaal category-ga saxda ah
        )
      : [];
  }, [items]);
  ...
  const itemsToDisplay = useMemo(() => {
    if (!selectedCategory) {
      return allTractorsForSaleItems;
    }

    const normalizedSelectedCategory = selectedCategory.toLowerCase();

    return allTractorsForSaleItems.filter((item: any) => {
      // Hubi in item.subcategory uu yahay Array oo uu ku jiro category-ga la doortay
      return (
        item.subcategory &&
  ...
  }, [allTractorsForSaleItems, selectedCategory]);
  ...
  const currentDisplayTitle = useMemo(() => {
    if (!selectedCategory) {
      return "All Tractors for Sale (Dhammaan Cagaf cagaf Beec ah)";
    }
