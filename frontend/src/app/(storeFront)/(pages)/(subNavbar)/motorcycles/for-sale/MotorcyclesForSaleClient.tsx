...
export default function MotorcyclesForSalePage({ initialData = [] }: { initialData?: any[] }) {
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      setSearchResults(
        results.filter((item: any) => {
          ...
          return (
            mainCat === "Motorcycle" &&
            (cat.includes("for sale") || cat.includes("iib"))
          );
        }),
      );
    }, 400);
  ...
  const baseItems = useMemo(() => {
    return items.filter((item) => {
      const cat = String(item?.category || "").toLowerCase();
      return cat.includes("for sale") || cat.includes("iib");
    });
  }, [items]);
  ...
  const finalItems = useMemo(() => {
    ...
    if (selectedSubcategory) {
      list = list.filter((item) => {
        ...
        return sub.toLowerCase().includes(selectedSubcategory.toLowerCase());
      });
    }
    ...
    <div className="container mx-auto px-2 py-2 space-y-3">
      ...
      <ContainerLinks>
        <CommonSubCategoryLinks
          items={nesCategories.MotorcyclesForNestedSub || []}
            ...
            <div className="text-xs font-medium text-gray-500 px-3 py-1 flex justify-between items-center">
              ...
              {!isLoading && (
                <span className="text-blue-700 font-bold">
                  {finalItems.length}{" "}
                  {t("motorcycles.unit", { defaultValue: "matooro" })}
                </span>
              )}
            </div>
          </ContainerLinks>
