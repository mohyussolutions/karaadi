...
export default function MotorcyclesForRent({ initialData = [] }: { initialData?: any[] }) {
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      setSearchResults(
        results.filter((item: any) => {
          ...
          return (
            mainCat === "Motorcycle" &&
            (cat.includes("rent") || cat.includes("kiro"))
          );
        }),
      );
    }, 400);
  ...
  const baseItems = useMemo(() => {
    return items.filter((item) => {
      const cat = String(item?.category || "").toLowerCase();
      return cat.includes("rent") || cat.includes("kiro");
    });
  }, [items]);
    ...
    <div className="container mx-auto px-2 py-2 space-y-3">
      ...
      <ContainerLinks>
        <CommonSubCategoryLinks
          items={nesCategories.MotorcycleRentNestedSub || []}
            ...
            <div className="text-xs font-medium text-gray-500 px-3 py-1 flex justify-between items-center">
              ...
              {!isLoading && (
                <span className="text-blue-700 font-bold">
                  {finalItems.length}{" "}
                  {t("motorcycles.rentUnit", {
                    defaultValue: "matooro kireysi ah",
                  })}
                </span>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5 sm:gap-3">
                {finalItems.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-20 text-gray-400 text-xs">
                    {t("noResults", {
                      defaultValue: "Ma jiraan matooro kireysi ah oo la helay.",
                    })}
                  </div>
                )}
              </div>
