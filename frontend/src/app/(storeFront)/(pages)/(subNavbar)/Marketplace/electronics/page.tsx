...
function ElectronicsLinks() {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
      ...
      const filteredElectronicsFromSearch = results.filter(
        (item: any) =>
          item.category && item.category.includes("Electronics & Home Goods"),
      );
      setSearchResults(filteredElectronicsFromSearch);
    }, 400);
  ...
  const subcategoryFilters = useMemo(() => {
    return ElectronicsNestedSub.map((item) => ({
  ...
  const currentDisplayTitle = useMemo(() => {
    ...
    if (!selectedSubcategory) {
      return "Dhamaan Qalabka Electronics & Guriga (All Electronics & Home Goods)";
    }
  ...
  if (error)
    return (
      <div className="text-red-500 p-4">Error loading electronics items.</div>
    );
              ...
              {itemsToDisplay.length > 0 ? (
                ...
                <div className="col-span-full text-center py-10 text-gray-500">
                  {selectedRegion || Object.keys(checkedCities).length > 0
                    ? `Ma jiro wax alaab Qalabka Electronics & Guriga ah ${selectedRegion ? `gobolka ${selectedRegion}` : ""}${Object.keys(checkedCities).length > 0 ? ` magaalooyinka ${Object.keys(checkedCities).join(", ")}` : ""}`
                    : query.trim()
                      ? `Ma jiro wax alaab Qalabka Electronics & Guriga ah la raadinayay "${query}"`
                      : "Ma jiro wax alaab Qalabka Electronics & Guriga ah oo la helay"}
                </div>
              )}
            </div>
          )}
...
export default ElectronicsLinks;
