...
export default function AntiquesAndArt() {
  ...
  const itemsToDisplay = useMemo(() => {
    let list = query.trim() ? searchResults : allAntiquesItems;
    ...
    <div className="container mx-auto px-4 pb-10">
      ...
      <div className="relative py-6">
        <div className="flex justify-center relative items-center">
          ...
          >
            {AntiquesAndArtNestedSub.map((filter) => (
              <button
                key={filter.title}
                onClick={() =>
                ...
                className={`flex-shrink-0 w-44 flex flex-col items-center justify-center text-center rounded-xl p-5 border transition-all duration-300 ${
                  selectedSubcategory === filter.title
                    ? "bg-amber-600 border-amber-600 shadow-lg scale-105 text-white"
                    : "bg-white border-gray-200 hover:border-amber-400 hover:shadow-md text-gray-900"
                }`}
              >
                ...
                <span
                  className={`text-[10px] uppercase mt-1 ${selectedSubcategory === filter.title ? "text-amber-100" : "text-gray-400"}`}
                >
                  ({filter.title})
                </span>
              </button>
        ...
        <aside className="md:w-1/3 sticky top-4 self-start">
          ...
          <div className="mt-4 bg-gray-50 rounded-xl p-2 border border-gray-100 shadow-sm">
            <SomaliMap
              ...
              items={allAntiquesItems}
            />
          </div>
        </aside>

        <main className="md:w-2/3 w-full">
          <div className="mb-6 text-sm font-medium text-gray-600 bg-amber-50 py-2 px-4 rounded-lg inline-block border border-amber-100">
            Waxaa la soo bandhigayaa{" "}
            <span className="text-amber-700 font-bold">
              {itemsToDisplay.length}
            </span>{" "}
            alaabta qadiimiga ah.
          </div>
