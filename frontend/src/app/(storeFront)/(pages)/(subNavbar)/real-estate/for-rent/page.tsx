...
function ForRent() {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
      ...
      const filteredRentalFromSearch = results.filter(
        (item: any) => item.category && item.category.includes("For Rent"),
      );
      setSearchResults(filteredRentalFromSearch);
    }, 400);
        ...
        if (selectedCities.length > 0) {
          ...
          for (const city of selectedCities) {
            ...
            const filteredByCategory = results.filter(
              (item: any) =>
                item.category && item.category.includes("For Rent"),
            );
              ...
              {itemsToDisplay.length > 0 ? (
                ...
                <div className="col-span-full text-center py-10 text-gray-500">
                  {selectedRegion || Object.keys(checkedCities).length > 0
                    ? `No rental properties found ${selectedRegion ? `in ${selectedRegion} region` : ""}${Object.keys(checkedCities).length > 0 ? ` in cities ${Object.keys(checkedCities).join(", ")}` : ""}`
                    : query.trim()
                      ? `No rental properties found for "${query}"`
