...
export default function FashionAndAccessories({ initialData = [] }: { initialData?: any[] }) {
  const { t } = useTranslation();
  const scrollRef = useRef<HTMLDivElement>(null);
  const { renderError } = useError();
  const { scroll } = usehandleHorizontalScroll(scrollRef);

  const [items, setItems] = useState<MarketplaceItem[]>(initialData as MarketplaceItem[]);
  ...
  const allFashionItems = useMemo(() => {
    return items.filter((item) => {
      const cat = Array.isArray(item.category)
        ? item.category
        : [item.category];
      return cat.some((c) => c === "Fashion & Accessories");
    });
  }, [items]);
  ...
  const ordered = useListingFeed(filteredItems, query, "Fashion & Accessories");
  ...
  const counts = useMemo(() => {
    ...
    allFashionItems.forEach((item) => {
      ...
      if (item.city) {
        ...
        cityCounts[cit] = cit ? (cityCounts[cit] || 0) + 1 : 0;
      }
    });
