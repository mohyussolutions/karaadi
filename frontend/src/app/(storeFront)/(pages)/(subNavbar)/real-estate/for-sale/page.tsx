...
import { RealEstateForSaleNestedSub } from "@/app/(links)/storeFrontLinks/nestedSubcategoryProperties";
...
function ForSale() {
  const subCategoryLinks = RealEstateForSaleNestedSub;
  ...
  const allForSaleItems = useMemo(() => {
    return Array.isArray(items)
      ? items.filter((item: any) => {
          ...
          return (
            cat.toLowerCase().includes("for sale") ||
            cat.toLowerCase().includes("iib")
          );
        })
      : [];
  }, [items]);
  ...
  const regionCityCounts = useMemo(() => {
    ...
    allForSaleItems.forEach((item: any) => {
      const format = (s: any) => {
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      const filtered = results.filter((item: any) => {
        const cat = Array.isArray(item.category)
          ? item.category.join(" ")
          : String(item.category || "");
        return cat.toLowerCase().includes("for sale");
      });
    ...
    <div className="container mx-auto px-4 pb-10">
      ...
      <div className="relative py-6">
        <div className="flex justify-center relative items-center">
          <button
            onClick={() => scroll("left")}
            className="absolute left-0 z-10 bg-white shadow-md p-3 rounded-full hover:bg-gray-100 transition-all border border-gray-100"
          >
            <FaChevronLeft />
          </button>
          <div
          ...
          <button
            ...
            className="absolute right-0 z-10 bg-white shadow-md p-3 rounded-full hover:bg-gray-100 transition-all border border-gray-100"
          >
            <FaChevronRight />
          </button>
        </div>
      ...
      <div className="mb-6 text-sm font-medium text-gray-600 bg-blue-50 py-2 px-4 rounded-lg inline-block border border-blue-100">
        Waxaa la helay{" "}
        <span className="text-blue-600 font-bold">{itemsToDisplay.length}</span>{" "}
        guryo iib ah.
      </div>
            ...
            {itemsToDisplay.length > 0
              ...
              : !isLoading && (
                  <div className="col-span-full text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 text-gray-500">
                    Ma jiro wax guryo iib ah oo la helay.
                  </div>
                )}
          </div>
        </main>
