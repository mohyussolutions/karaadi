...
function ForSale() {
  ...
  const allForSaleItems = useMemo(() => {
    return Array.isArray(items)
      ? items.filter((item: any) => item.category.includes("For Sale"))
      : [];
  }, [items]);
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
      ...
      const filteredSaleFromSearch = results.filter(
        (item: any) => item.category && item.category.includes("For Sale"),
      );
      setSearchResults(filteredSaleFromSearch);
    }, 400);
        ...
        if (selectedCities.length > 0) {
          ...
          for (const city of selectedCities) {
            ...
            const filteredByCategory = results.filter(
              (item: any) =>
                item.category && item.category.includes("For Sale"),
            );
              ...
              {itemsToDisplay.length > 0 ? (
                ...
                <div className="col-span-full text-center py-10 text-gray-500">
                  {selectedRegion || Object.keys(checkedCities).length > 0
                    ? `No properties found ${selectedRegion ? `in ${selectedRegion} region` : ""}${Object.keys(checkedCities).length > 0 ? ` in cities ${Object.keys(checkedCities).join(", ")}` : ""}`
                    : query.trim()
