...
function RealEstateLinks() {
  ...
  return (
    <div className="w-full max-w-screen-xl mx-auto px-2 py-2 space-y-4 overflow-x-hidden">
      ...
      <ContainerLinks>
        <SubCategoryList data={realEstateSubCategories} />
      </ContainerLinks>
      <ContainerLinks>
        <WantSell />
      </ContainerLinks>
          ...
          <ContainerLinks>
            <div className="text-sm font-medium text-gray-600 px-3 py-1 flex justify-between items-center">
              ...
              <span className="text-blue-700 font-bold">
                {displayItems.length}{" "}
                {t("properties", { defaultValue: "guryo" })}
              </span>
            </div>
          </ContainerLinks>
          <ContainerLinks>
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5 sm:gap-3">
              {visibleItems.length > 0 ? (
                visibleItems.map((item: any, index: number) => {
                  ...
                  const routePrefix = getRoute("real-estate");
                ...
                <div className="col-span-full text-center py-12 text-gray-400 text-sm">
                  {t("noResults", {
                    defaultValue: "Lama helin guryo waafaqsan raadintaada.",
                  })}
                </div>
              )}
            </div>
