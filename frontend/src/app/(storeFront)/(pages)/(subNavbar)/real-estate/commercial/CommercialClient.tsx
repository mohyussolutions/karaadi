...
export default function Commercial({ initialData = [] }: { initialData?: any[] }) {
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      setSearchResults(
        results.filter((item: any) => {
          ...
          return cat.includes("commercial") || cat.includes("ganacsi");
        }),
      );
    }, 400);
    ...
    <div className="container mx-auto px-2 py-2 space-y-3">
      ...
      <ContainerLinks>
        <CommonSubCategoryLinks
          items={nesCategories.RealEstateCommercialNestedSub || []}
            ...
            <div className="text-xs font-medium text-gray-500 px-3 py-1 flex justify-between items-center">
              ...
              {!isLoading && (
                <span className="text-blue-700 font-bold">
                  {finalItems.length}{" "}
                  {t("commercialItems", { defaultValue: "hanti ganacsi" })}
                </span>
              )}
            </div>
          </ContainerLinks>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-2">
                {finalItems.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-20 text-gray-400 text-xs">
                    {t("noResults", {
                      defaultValue:
                        "Ma jiraan hanti ganacsi oo waafaqsan raadintaada.",
                    })}
                  </div>
                )}
              </div>
