...
function Commercial() {
  ...
  const allCommercialItems = useMemo(() => {
    return items.filter((item: any) => {
      ...
      return (
        ...
        cat.toLowerCase().includes("ganacsi")
      );
    });
  }, [items]);
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      const filtered = results.filter((item: any) => {
        const cat = Array.isArray(item.category)
          ? item.category.join(" ")
          : String(item.category || "");
        return cat.toLowerCase().includes("commercial");
      });
  ...
  return (
    <div className="container mx-auto px-4 pb-10">
      ...
      <div className="px-4 text-sm text-gray-700 mb-6 bg-blue-50 py-2 px-4 rounded-lg inline-block border border-blue-100">
        Waxaa la helay{" "}
      ...
      <div className="flex flex-col-reverse md:flex-row gap-8 pt-2">
        <aside className="md:w-1/3 sticky top-4 self-start space-y-6">
          <LocationSelector
              ...
              {itemsToDisplay.length > 0
                ...
                : !isLoading && (
                    <div className="col-span-full text-center py-20 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 text-gray-500">
                      Ma jiro wax hanti ganacsi ah oo la helay.
                    </div>
                  )}
            </div>
          )}
