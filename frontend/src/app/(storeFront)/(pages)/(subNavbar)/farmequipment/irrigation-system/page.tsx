...
export default function IrrigationSystem() {
  ...
  const subCategoryLinks = (categories.IrrigationSystemNestedSub || []) as TraktorSubCategoryItem[];
  ...
  const baseItems = useMemo(() => {
    ...
    return items.filter((item: FarmEquipment) => {
      ...
      return (
        searchTarget.includes("irrigation") ||
        searchTarget.includes("water pump") ||
        searchTarget.includes("sprinkler") ||
        searchTarget.includes("biyo")
      );
    });
  }, [items]);
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      const filtered = (results as any[]).filter((item) => {
        ...
        return (
          ...
          sub.includes("irrigation") ||
          cat.includes("water pump") ||
  ...
  const itemsToDisplay = useMemo(() => {
    ...
    if (selectedSubcategory) {
      ...
      list = list.filter((item: FarmEquipment) => {
        ...
        return sub.toLowerCase().includes(normalized) || title.includes(normalized);
      });
    }
  ...
  }, [query, searchResults, baseItems, selectedSubcategory, selectedRegion, checkedCities]);
            ...
            <div className="text-xs font-medium text-gray-500 px-3 py-1 flex justify-between items-center">
              ...
              {!isLoading && (
                <span className="text-emerald-700 font-bold">
                  ...
                  {t("items", { defaultValue: "nidaamka waraabinta" })}
                </span>
              )}
            </div>
          </ContainerLinks>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5 sm:gap-3">
                {itemsToDisplay.length > 0 ? (
                  ...
                  <div className="col-span-full text-center py-20 text-gray-400 text-xs font-medium">
                    {t("noResults", {
                      defaultValue: "Lama helin nidaamka waraabinta oo waafaqsan raadintaada.",
                    })}
                  </div>
                )}
              </div>
