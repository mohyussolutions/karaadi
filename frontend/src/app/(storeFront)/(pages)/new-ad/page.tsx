...
const filteredCategories = (allCategories as MainCategory[]).filter(
  (c) => !c.logo && !["Smartsuuq", "mohyus", "smart suuq", "jobs"].includes((c.name || c.key || "").toLowerCase()),
);
...
export default function NewAdPage() {
  ...
  const selectedCategoryObj = filteredCategories.find((c) => c.name === selectedCategory);

  return (
    <div className="min-h-screen bg-gray-50" style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 4rem)" }}>
          ...
          <div>
            ...
            {!selectedCategory && (
              <p className="text-[11px] text-gray-400 font-medium leading-none mt-0.5">
                {t("createAd.selectCategory", "Select a category to get started")}
              </p>
            )}
          </div>
        </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3.5 border-b border-gray-100 last:border-b-0 text-sm font-semibold transition-colors touch-manipulation select-none ${
                    isSelected ? "bg-blue-600 text-white" : "text-gray-700 hover:bg-gray-50 active:bg-gray-100"
                  }`}
                >
                  <span className="text-lg leading-none flex-shrink-0">{category.icon}</span>
                  <span className="flex-1 text-left">{label}</span>
                  <ChevronRight className={`w-4 h-4 flex-shrink-0 transition-opacity ${isSelected ? "opacity-70" : "opacity-0"}`} />
                </button>
              );
            })}
          </nav>
            ...
            {!selectedCategory ? (
              <div className="flex flex-col items-center justify-center py-28 text-center bg-white rounded-2xl border border-gray-200 shadow-sm">
                ...
                <p className="text-sm text-gray-400">
                  {t("createAd.selectCategoryHint", "Choose from the list on the left")}
                </p>
              </div>
            ) : (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="font-semibold text-gray-900 text-sm block">
                            {item.labelKey ? t(item.labelKey, { defaultValue: item.title || item.name }) : item.title || item.name}
                          </span>
                          {item.description && (
                            <span className="text-xs text-gray-400 block mt-0.5">{item.description}</span>
                          )}
                        </div>
                      </div>
                ...
                {filteredCategories.map((category) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="font-bold text-gray-800 text-sm text-center leading-snug">{label}</span>
                    </button>
                  );
                })}
              </div>
