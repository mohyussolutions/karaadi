...
import { FiInbox, FiTrash2, FiCheckCircle, FiArrowRight } from "@/app/utils/icons";
import { FiMapPin, FiTag, FiDollarSign, FiEdit, FiPackage, FiInfo, FiPlus } from "react-icons/fi";
...
const HIDDEN_FIELDS = new Set([
  "images", "image", "id", "feeAmount", "_id",
  "createdAt", "updatedAt", "__v", "title", "price",
  "category", "subCategory", "city", "region", "description",
]);
...
export default function CartContent() {
  ...
  useEffect(() => {
    ...
    if (!ownerId && currentUserId) dispatch(setOwnerId(currentUserId));
  ...
  const handleGoToPlan = async () => {
    if (!hasActive) { toast.error(t("cart.noItem", "Select an item first")); return; }
    ...
    try {
      ...
      if (plansRes && Array.isArray(plansRes) && !hasPlan) {
        ...
        if (defaultPlan && config) {
          dispatch(setPlan({ ...defaultPlan, id: defaultPlan.key, price: Number(config[defaultPlan.key]) || 0 }));
        }
      }
      router.push("/plan");
    } catch {
  ...
  const handleGoToPayment = () => {
    if (!hasActive) { toast.error(t("cart.noItem", "Select an item first")); return; }
    if (!hasPlan) { toast.error(t("cart.noPlan", "Please select a plan first")); return; }
    router.push("/payment");
  };
    ...
    <div className="mt-8 w-full max-w-3xl mx-auto px-4 pb-16 space-y-5">
        ...
        <div className="flex items-center gap-6">
          {hasActive && (
            <div className="text-center">
              <div className="text-2xl font-black">${totalFee.toLocaleString()}</div>
              <div className="text-[10px] font-black uppercase opacity-70">Selected Total</div>
            </div>
          )}
          {hasPlan && (
            <div className="text-center">
              ...
              <div className="text-[10px] font-black uppercase opacity-70">Plan</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
          ...
          >
            <FiPlus size={13} /> {t("cart.addAnother", "Add Another")}
          </button>
        </div>
        ...
        <div className="p-3 space-y-2">
          {allItems.map((item) => {
            ...
            const extraFields = Object.entries(item).filter(
              ([k, v]) => !HIDDEN_FIELDS.has(k) && v !== null && v !== undefined && v !== "" && !Array.isArray(v),
            );
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="flex-shrink-0 flex flex-col items-end gap-1.5">
                        ...
                        >
                          {isActive && <HiOutlineCheckCircle className="text-white" size={13} />}
                        </div>
                      </div>
                    </div>
                    ...
                    {Array.isArray(item.images) && item.images.length > 0 && (
                      <div className="flex gap-1.5 overflow-x-auto pb-1">
                        {item.images.map((src, idx) => (
                          <img key={idx} src={src} alt="" className="w-14 h-14 rounded-lg object-cover flex-shrink-0 border border-blue-100" />
                        ))}
                      </div>
                    )}
                  </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <FiEdit size={13} />
              {hasPlan ? t("cart.change", "Change") : t("cart.select", "Select")}
            </button>
            {hasPlan && (
              <button
          ...
          {hasPlan ? (
            <div className="flex items-center justify-between p-4 bg-green-50 rounded-xl border border-green-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-black text-gray-900 text-sm">{draftPlan.label}</p>
                  <p className="text-xs text-gray-500">{draftPlan.days} days active listing</p>
                </div>
              </div>
      ...
      {hasActive && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-4 space-y-2.5 text-sm">
            ...
            <div className="flex justify-between">
              <span className="text-gray-500 font-medium">{t("cart.listingFee", "Listing Fee")}</span>
              <span className="font-bold text-gray-800">${itemFee.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500 font-medium">{t("cart.planPrice", "Plan Price")}</span>
              <span className="font-bold text-gray-800">{hasPlan ? `$${planPrice.toLocaleString()}` : "—"}</span>
            </div>
            <div className="flex justify-between items-center pt-3 border-t border-gray-100 font-black text-base">
              <span className="text-gray-900">{t("cart.totalFee", "Total Due")}</span>
              <span className="text-blue-600 text-xl">${totalFee.toLocaleString()}</span>
            </div>
          </div>
          <div className="px-5 pb-5 flex flex-col sm:flex-row gap-3">
            ...
            >
              {hasPlan ? t("cart.changePlan", "Change Plan") : t("cart.selectPlan", "Select Plan")}
            </button>
            <button
            ...
            >
              {!hasPlan ? t("cart.selectPlanFirst", "Select Plan First") : t("cart.goToPayment", "Proceed to Payment")}
