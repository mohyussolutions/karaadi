...
export default function CarsPage() {
  ...
  useEffect(() => {
    setItems(loadBackofficeItems("cars"));
  }, []);

  function handleDelete(id: string) {
    if (!confirm("Delete this car?")) return;
    deleteBackofficeItem("cars", id);
    setItems(loadBackofficeItems("cars"));
  }
    ...
    <div className="py-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <Link href="/Backoffice/cars/creating" className="px-3 py-2 bg-blue-600 text-white rounded">Create</Link>
          <Link href="/Backoffice" className="px-3 py-2 bg-gray-200 rounded">Back</Link>
        </div>
      </div>

      {items.length === 0 ? (
        <div className="text-gray-500">No cars yet.</div>
      ) : (
          ...
          {items.map((it) => (
            <div key={it.id} className="p-3 border rounded flex items-start justify-between">
              <div>
                ...
                <div className="text-sm text-gray-600">{it.brand} {it.vehicleModel}</div>
                <div className="text-sm mt-2">Price: {it.price}</div>
              </div>
              <div className="flex flex-col gap-2">
                <Link href={`/Backoffice/cars/edit?id=${it.id}`} className="px-2 py-1 bg-yellow-200 rounded">Edit</Link>
