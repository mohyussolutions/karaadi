...
function getCategoryInfo(payment: Payment): {
  ...
  if (payment.boatId)
    return {
      ...
      title: payment.boat?.title || "Boat listing",
      image: payment.boat?.images?.[0],
    };
  if (payment.carId)
    return {
      name: "Car",
      icon: <FaCar className="text-green-500" />,
      title: payment.car?.title || "Car listing",
      image: payment.car?.images?.[0],
    };
  if (payment.realEstateId)
    return {
      name: "Real Estate",
      icon: <FaHome className="text-purple-500" />,
      title: payment.realEstate?.title || "Property listing",
      image: payment.realEstate?.images?.[0],
    };
  if (payment.motorcycleId)
    return {
      name: "Motorcycle",
      icon: <FaMotorcycle className="text-orange-500" />,
      title: payment.motorcycle?.title || "Motorcycle listing",
      image: payment.motorcycle?.images?.[0],
    };
  if (payment.farmequipmentId)
    return {
      name: "Farm Equipment",
      icon: <FaTractor className="text-yellow-700" />,
      title: payment.farmequipment?.title || "Equipment listing",
      image: payment.farmequipment?.images?.[0],
    };
  if (payment.marketplaceId)
    return {
      name: "Marketplace",
      icon: <FaShoppingBag className="text-pink-500" />,
      title: payment.marketplace?.title || "Marketplace listing",
      image: payment.marketplace?.images?.[0],
    };
  if (payment.jobId)
    return {
      name: "Job",
      icon: <FaBriefcase className="text-indigo-500" />,
      title: payment.job?.title || "Job listing",
    };
...
function MethodBadge({ method }: { method?: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-bold border ${m.color}`}
    >
      {m.label}
    </span>
  );
              ...
              <table className="w-full text-left table-fixed">
                <thead>
                  <tr className="border-b border-gray-100 text-[11px] text-gray-400 uppercase tracking-wider">
                    <th className="py-3 px-4 font-medium w-[18%]">
                      {t("adminTable.user")}
                    </th>
                    <th className="py-3 px-4 font-medium w-[16%]">
                      {t("adminTable.paidFrom")}
                    </th>
                    <th className="py-3 px-4 font-medium w-[26%]">
                      {t("adminTable.listing")}
                    </th>
                    <th className="py-3 px-4 font-medium w-[12%]">
                      {t("adminTable.amount")}
                    </th>
                    <th className="py-3 px-4 font-medium w-[11%]">
                      {t("adminTable.method")}
                    </th>
                    <th className="py-3 px-4 font-medium w-[10%]">
                      {t("adminTable.status")}
                    </th>
                    <th className="py-3 px-4 font-medium w-[7%]">
                      {t("adminTable.date")}
                    </th>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
                <span className="text-xs text-gray-400">
                  {t("adminTable.page")} {page} of {totalPages} · {total}{" "}
                  {t("adminTable.total").toLowerCase()}
                </span>
...
function StatCard({
  ...
  return (
    ...
    >
      <div className={`inline-flex p-2 rounded-xl ${iconBg} mb-2.5`}>
        {icon}
      </div>
