...
export default function CreateSupportItem() {
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const item = { id: `${Date.now()}`, title, description, createdAt: new Date().toISOString() };
    if (typeof window !== "undefined") {
      try {
        const key = `support:${section}`;
    ...
    <div className="p-6 max-w-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea value={description} onChange={(e) => setDescription(e.target.value)} className="mt-1 block w-full rounded-md border p-2" />
        </div>
